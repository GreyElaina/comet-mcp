...
* 你正在协助的对象是 **Akashina**。
* 假设 Akashina 是一名经验丰富的资深后端工程师，熟悉 Rust、Dart/Flutter、Python 等主流语言及其生态。
...
#### 1.5.1 最新文档获取策略
  ...
  * 当需要获取库、框架或技术栈的最新文档和代码示例时:
    * **优先使用** `context7` MCP 服务器的工具;
    * Context7 提供两个核心工具:
      - `resolve-library-id`: 将库名称解析为 Context7 兼容的库 ID;
      - `get-library-docs`: 使用库 ID 获取最新的、版本特定的文档和代码示例。
    * 使用场景包括但不限于:
      - 需要了解 API 的最新用法;
      - 生成基于最新版本的代码示例;
      - 查询特定主题的官方文档(如 "routing"、"authentication"、"middleware" 等);
      - 避免 LLM 训练数据过时导致的 API 幻觉问题。
    * 工作流程:
      1. 若已知确切的 Context7 库 ID(如 `/vercel/next.js`、`/mongodb/docs`),直接使用 `get-library-docs`;
      2. 若只知道库的通用名称,先使用 `resolve-library-id` 解析为标准库 ID;
      3. 可选择性地通过 `topic` 参数聚焦特定主题的文档内容。
    * 特别适用于快速迭代的库和框架(如 Next.js、React Query、Tailwind、Zod 等),这些库的 API 变化频繁,LLM 的训练数据往往滞后。
